...
const MULTIMODAL_MODEL = "gemini-3.1-flash-lite";
...
async function askAI(userMessage, historyKey, guildId, attachments = [], retries = 2) {
  ...
  const mediaParts = fileParts.filter((p) => p.inlineData);

  // มีรูป/PDF แนบมา → ใช้ Gemini Flash (multimodal) แบบ one-shot
  if (mediaParts.length > 0) {
    const parts = [{ text: userMessage || "ช่วยอธิบายไฟล์ที่แนบมาให้หน่อยครับ" }, ...mediaParts];
    ...
    saveMessage(historyKey, "user", userMessage || "[ส่งรูปภาพ/ไฟล์]");
    saveMessage(historyKey, "model", replyText);
    return replyText;
  }

  // ข้อความปกติ (อาจมีไฟล์ข้อความแนบ → ต่อเข้า prompt) → ใช้ Gemma (Unlimited) เหมือนเดิม
  const textExtras = fileParts.filter((p) => p.text).map((p) => p.text).join("");
  const finalMessage = userMessage + textExtras;
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (err?.status === 500 && i < retries) {
        console.log(`⚠️ Gemma 500 error, retry ${i + 1}/${retries}...`);
...
หรือจะ **mention บอท** / **DM** ตรงๆ ก็คุยได้เลยครับ!`);
    ...
    if (commandName === "image") {
      ...
      try {
        ...
        if (!imagePart) return interaction.editReply("❌ สร้างรูปไม่สำเร็จ ลองใหม่อีกครั้งครับ");
        ...
        return interaction.editReply({ content: `🎨 **${prompt}**${textPart ? `\n${textPart.text}` : ""}`, files: [attachment] });
      } catch (err) {
